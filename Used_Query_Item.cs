...
{
    public partial class Used_Query_Item : Form
    {
        DataClasses1DataContext mob_sys = new DataClasses1DataContext();
        DataTable dt = new DataTable();
        public Used_Query_Item()
        {
            InitializeComponent();
        }

        private void Used_Query_Item_Load(object sender, EventArgs e)
        {
        ...
        private void Btn_Show_Store_Items_Click(object sender, EventArgs e)
        {
            label8.Text = "";
            dt.Clear();
            var items = mob_sys.Used_Items.Select(s => new { s.Name, s.Count, s.Recieve_Price, s.Sale_Price });
            var maxid = mob_sys.Used_Items.Max(s => s.ID);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (textBox1.Text != "")
                {
                    var items = mob_sys.Used_Items.Where(s => s.Name == textBox1.Text).Select(s => new { s.Count, s.Recieve_Price, s.Sale_Price });
                    foreach (var item in items)
                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {   
                Used_View_report_store_items v = new Used_View_report_store_items();
                v.Show();
            }
            else
            {
