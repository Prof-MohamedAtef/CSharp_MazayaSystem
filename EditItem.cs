...
namespace Mazaya_3G_return
{
    public partial class EditItem : Form
    {
        ...
        {
            var allitems = mob_sys.Items.Select(s => new { s.Name, s.ID }).ToList();
            foreach (var item in allitems)
            {
        ...
        public void load_detailsbycode(int x)
        {
            ...
            if (isitemexit(" ", x))
            {
                ...
                var sale_details = mob_sys.Items.Where(s => s.ID == x).Select(s => new
                {
                    s.count,
                    s.Name,
                    s.Notes,
                ...
                var comp_type_id = mob_sys.Items.Where(s => s.ID == x).Select(s => new { s.Name, s.Producing_company_id, s.Type }).SingleOrDefault();
            ...
            try
            {
                if (txt_sale_price.Text != "" && txt_sale_price.Text != "0" && txt_sale_price.Text != "0.00")
                {
                    if (txt_code_search.Text != "")
                    {
                        ...
                        mob_sys.update_item_saleprice(id, price);
                    ...
                    {
                        var name_id = mob_sys.Items.Where(s => s.Name == textBox1.Text).Select(s => s.ID).SingleOrDefault();
                       
                        var price = float.Parse(txt_sale_price.Text);
                        mob_sys.update_item_saleprice(name_id, price);
                        label9.Text = "تم التسعير";
                    }
                }
