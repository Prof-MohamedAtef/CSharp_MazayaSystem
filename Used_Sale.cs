...
{
    public partial class Used_Sale : Form
    {
        ...
        private void Used_Sale_Load(object sender, EventArgs e)
        {
            textBox2.Focus();
        ...
        public void reset_form() 
        {
            ...
            var allitems = mob_sys.Used_Items.Select(s => new { s.Name, s.Count,s.Screen_Title_id }).ToList();
            ...
            try
            {
                var maxid = mob_sys.Used_Sale_Checks.Max(s => s.ID);
        ...
        public void ins_sal_details()
        {
            try
            {
                var sale_id = mob_sys.Used_Sale_Checks.Max(s => s.ID);
                if (dataGridView1.RowCount != -1)
                {
                    for (int i = 0; i < dataGridView1.RowCount ; i++)
                    {
                        var max_sal_id = mob_sys.Used_Sale_Check_details.Max(s => s.ID);
                        ...
                        if (Combo__tax_Discount.SelectedIndex == 0)
                        {
                            ...
                            mob_sys.Used_ins_Sale_check_detail(max_sal_id + 1, int.Parse(row.Cells[1].Value.ToString()),
                        ...
                        else if (Combo__tax_Discount.SelectedIndex == 1)
                        {
                            ...
                            var after_dis = (pa_price * val) / 100;
                            var result = pa_price + after_dis;
                            mob_sys.Used_ins_Sale_check_detail(max_sal_id + 1, int.Parse(row.Cells[1].Value.ToString()), int.Parse(row.Cells[3].Value.ToString())
                            , double.Parse(row.Cells[5].Value.ToString()), row.Cells[6].Value.ToString(), sale_id, double.Parse(row.Cells[4].Value.ToString()),result);
                        }
                        else
                            mob_sys.Used_ins_Sale_check_detail(max_sal_id + 1, int.Parse(row.Cells[1].Value.ToString()), int.Parse(row.Cells[3].Value.ToString())
                ...
                if (dataGridView1.RowCount != -1)
                {
                    for (int i = 0; i < dataGridView1.RowCount ; i++)
                    {
                        ...
                        if (Combo__tax_Discount.SelectedIndex == 0)
                        {
                            ...
                            var after_dis = (pa_price * val) / 100;
                            var result = pa_price - after_dis;
                            mob_sys.Used_ins_Sale_check_detail(i + 1, int.Parse(row.Cells[1].Value.ToString()), int.Parse(row.Cells[3].Value.ToString())
                        ...
                        else if (Combo__tax_Discount.SelectedIndex == 1)
                        {
                            ...
                            mob_sys.Used_ins_Sale_check_detail(i + 1, int.Parse(row.Cells[1].Value.ToString()), int.Parse(row.Cells[3].Value.ToString())
                             ...
                             mob_sys.Used_sub_item_count(int.Parse(row.Cells[3].Value.ToString()), int.Parse(row.Cells[1].Value.ToString()));
                    }
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
       {
                ...
                if (textBox10.Text != "")
                {
                        try
                        {
                            ...
                            if (txt_value.Text !="")
                            {
                                ...
                                if (Combo__tax_Discount.SelectedIndex == 0)
                                {
                                    ...
                                    mob_sys.Used_insert_sale_check(maxid + 1, System.DateTime.Now.Date, textBox2.Text, Class1.TheID, 
                        ...
                        catch (Exception)
                        {
                            if (txt_value.Text != "")
                            {
                                ...
                                if (Combo__tax_Discount.SelectedIndex == 0)
                                {
                                        mob_sys.Used_insert_sale_check(1, System.DateTime.Now.Date, textBox2.Text, Class1.TheID,
                        ...
                        if (textBox5.Text != "")
                        {
                            if (linkLabel1.Enabled == false)
                            {
                                try
                                {
                                    ...
                                    var itemdetails = mob_sys.Used_Items.Where(s => s.Name == x).Select(s => new { s.ID, s.Name, s.Sale_Price }).SingleOrDefault();
                        ...
                        if (dialogResult == DialogResult.Yes)
                        {
                            foreach (DataRow item in dt.Rows)
                            {
                                ...
                                if (item["اسم الصنف"].ToString() == txt_Add_Item.Text)
                                {
                                    ...
                                    var itemdetailss = mob_sys.Used_Items.Where(s => s.Name == xx).Select(s => new { s.ID, s.Name, s.Sale_Price }).SingleOrDefault();
        ...
        {
            var itemdetails = mob_sys.Used_Items.Where(s => s.Name == txt_Add_Item.Text).Select(s => new { s.ID, s.Name, s.Count,s.Sale_Price ,s.Recieve_Price}).SingleOrDefault();
            try
            {
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            Used_EditItem edit = new Used_EditItem();
            edit.ShowDialog();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                Class1.Sale_id = mob_sys.Used_Sale_Checks.Max(s => s.ID);
                Used_View_report_sale_check v = new Used_View_report_sale_check();
                v.Show();
            }        
        }
