    ...
    public partial class used : Form
    {
        ...
        private void used_Load(object sender, EventArgs e)
        {
           ...
           if (linklablepress)
           {
            ...
            reset_form();
            textBox2.Text = "مستعمل";
            
        }
        public void reset_form() {
            try
            {
                var maxid = mob_sys.Used_Purchase_checks.Max(s => s.ID);
       ...
       public void load_compo2()
       {
        ...
        public bool isitemexit(string x) {
            try
            {
                var allitems = mob_sys.Used_Items.Select(s => new { s.Name}).ToList();
                foreach (var item in allitems)
                {
                    if (item.Name == x )
                    {
            ...
            if (dataGridView1.RowCount != -1)
            {
                for (int i = 0; i < dataGridView1.RowCount; i++)
                {
                    ...
                    if (c!= " ")
                    {
                        try
                        {
                            var max_item_id = mob_sys.Used_Items.Max(s => s.ID);
                            if (isitemexit(row.Cells[2].Value.ToString()))
                            {
                                 var prev_count = mob_sys.Used_Items.Where(s => s.Name == item_name).Select(s => s.Count).SingleOrDefault();
                                 var id_item = mob_sys.Used_Items.Where(s => s.Name == item_name).Select(s => new { s.ID ,s.Recieve_Price}).SingleOrDefault();
                                 ...
                                 if (dialogResult == DialogResult.Yes)
                                 {
                                     mob_sys.update_Used_itemcouunt2(id_item.ID, item_count + prev_count,p);
                                 }
                                 else if (dialogResult == DialogResult.No)
                                 { 
                                 //here ... reject price and accept quantity
                                     mob_sys.update_Used_itemcouunt2(id_item.ID, item_count + prev_count, id_item.Recieve_Price);
                            ...
                            {
                                mob_sys.ins_Used_Items(max_item_id + 1, row.Cells[2].Value.ToString(), type_id,
                        ...
                        {
                            mob_sys.ins_Used_Items(1, row.Cells[2].Value.ToString(), type_id,
                            int.Parse(row.Cells[5].Value.ToString())
                            , double.Parse(row.Cells[6].Value.ToString()), 0.00, row.Cells[8].Value.ToString(), comp_id, 0);
                        }
                    }
                        ...
                        try
                        {
                           ...
                           //--------
                            ...
                            { 
                                ...
                                mob_sys.ins_Used_Items(max_item_id + 1, row.Cells[2].Value.ToString(), 0,
                                int.Parse(row.Cells[5].Value.ToString())
                        ...
                        catch (Exception)
                        {
                            var screen = row.Cells[3].Value.ToString();
                            var screen_id = mob_sys.Screen_Titles.Where(s => s.Screen_Title1 == screen).Select(s => s.ID).SingleOrDefault();
                            mob_sys.ins_Used_Items(1, row.Cells[2].Value.ToString(), 0,
                            int.Parse(row.Cells[5].Value.ToString())
                            , double.Parse(row.Cells[6].Value.ToString()), 0.00, row.Cells[8].Value.ToString(), 0, screen_id);
                        }
                    }
                    ...
                    try
                    {
                        var max_purch_details_id = mob_sys.Used_Purchase_Check_details.Max(s => s.ID);
                        var max_purch_id = mob_sys.Used_Purchase_checks.Max(s => s.ID);
                        var id_itemm = mob_sys.Used_Items.Where(s => s.Name == item_name).Select(s => s.ID).SingleOrDefault();
                        mob_sys.Used_ins_purchase_check_detail(max_purch_details_id + 1, id_itemm,
                        int.Parse(row.Cells[5].Value.ToString())
                    ...
                    catch (Exception)
                    {
                        ...
                        var max_purch_id = mob_sys.Used_Purchase_checks.Max(s => s.ID);
                        var id_itemm = mob_sys.Used_Items.Where(s => s.Name == item_namee).Select(s => s.ID).SingleOrDefault();
                        mob_sys.Used_ins_purchase_check_detail(1, id_itemm,
                        int.Parse(row.Cells[5].Value.ToString())
                        , double.Parse(row.Cells[7].Value.ToString()), row.Cells[8].Value.ToString(), max_purch_id);
                    }
                }
                    ...
                    if (textBox11.Text != "")
                    {
                        ...
                        try
                        {
                             ...
                             if (txt_value.Text !="")
                            {
                                ...
                                {
                                    mob_sys.insert_Used_Purchase_check(max, d, subname, sub_comp, Class1.TheID, egmaly, n, diff, val, 0.00, remain);
                                }
                                else 
                                {
                                    mob_sys.insert_Used_Purchase_check(max, d, subname, sub_comp, Class1.TheID, egmaly, n, diff, 0.00, val, remain);
                                }
                            }
                            else
                            {
                                mob_sys.insert_Used_Purchase_check(max, d, subname, sub_comp, Class1.TheID, egmaly, n, diff, 0.00, 0.00, 0.00);
                            }
                            ins_item();
                        ...
                        catch (Exception)
                        {
                            ...
                            if (txt_value.Text != "")
                            {
                                ...
                                if (Combo__tax_Discount.SelectedIndex == 0)
                                {
                                    mob_sys.insert_Used_Purchase_check(1, d, subname, sub_comp, Class1.TheID, egmaly, n, diff, val, 0.00, remain);
                                }
                                else
                                {
                                    mob_sys.insert_Used_Purchase_check(1, d, subname, sub_comp, Class1.TheID, egmaly, n, diff, 0.00, val, remain);
                                }
                            }
                            else
                            {
                                mob_sys.insert_Used_Purchase_check(1, d, subname, sub_comp, Class1.TheID, egmaly, n, diff, 0.00, 0.00, 0.00);
                            }
                            ins_item();
                            label11.Text = "تم عمل الفاتوره";
                            button1.Text = "تم الحفظ";
            ...
            if (comboBox4.SelectedIndex == 0)/////موبايلاااااااااااااااااااااااااااااات
            {
                if (textBox13.Text != "" && textBox15.Text != "" && comboBox5.SelectedIndex != -1 && comboBox6.SelectedIndex != -1&&textBox16.Text!="")
                {
                    if (!ifexist_item(textBox15.Text))
                    {
                        ...
                        try
                        {
                            var max_id = mob_sys.Used_Items.Max(s => s.ID);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "تم الحفظ")
            {
                //printPreviewDialog1.ShowDialog();
                //i_print = 0;
                Class1.Purch_id = mob_sys.Used_Purchase_checks.Max(s => s.ID);
                Used_View_purch_report_byID v = new Used_View_purch_report_byID();
                v.Show();
            }
            //printPreviewDialog1.ShowDialog();
            //i_print = 0;
        ...
        private void used_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)13)
            {
                textBox2.Focus();
        ...
        private void backupYourDataToolStripMenuItem1_Click(object sender, EventArgs e)
        {
        ...
        private void مرتجعمستعملToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        {
            Used_Query_Item uqi = new Used_Query_Item();
            uqi.ShowDialog();
        }
        ...
        private void مردوداتمبيعاتToolStripMenuItem1_Click(object sender, EventArgs e)
        {
