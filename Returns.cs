...
namespace Mazaya_3G_return
{
    public partial class Returns : Form
    {
        ...
        public void reset_form()
        {
                ...
                try
                {
                    var max_check_id = mob_sys.Returns_checks.Max(s => s.ID);
        ...
        public bool ischeckexist(int i)
        {
            try
            {
                var allitems = mob_sys.Sale_checks.Select(s => s.ID).ToList();
        ...
        public void auto_complete(int salecheck_id)
        {
            ...
            var allitems = mob_sys.Sale_check_details.Where(s => s.Sale_check_id == salecheck_id).Select(s => s.Item_ID).ToList();

            foreach (var item in allitems)
            {
                var item_name = mob_sys.Items.Where(s => s.ID == item).Select(s => new { name = s.Name, id = s.ID }).SingleOrDefault();
        ...
        public void autocomplete_names() {
            ...
            var allnames = mob_sys.Sale_checks.Select(s => new { s.Sale_To,s.ID }).ToList();
        ...
        public void after_press_id_enter()
        {
            ...
            var name = mob_sys.Sale_checks.Where(s => s.ID == id).Select(s => new { s.Sale_To, s.Total_price, s.remaining_val, s.difference_col }).SingleOrDefault();
            ...
            if (textBox13.Text != "")
            {
                if (int.Parse(textBox13.Text) <= int.Parse(textBox5.Text))
                {
                    ...
                    if (!ifexist_item(txt_Add_Item.Text))
                    {
                        ...
                        var name_id = mob_sys.Items.Where(s => s.Name == x).Select(s => s.ID).SingleOrDefault();
                        var x2=textBox11.Text;
                        var sale_price=mob_sys.Sale_checks.Where(s=>s.Sale_To==x2).Select(s=>s.difference_col).SingleOrDefault();
                        ...
                        txt_Add_Item.Text = textBox5.Text = textBox7.Text = textBox13.Text = textBox12.Text =
                        textBox10.Text = "";
                        sum_total();
                        // هنا عاوزين الvalue الموجودة 
                        // فى العمود difference_col 
                        // بجدول Sale_Check
                        // فى textbox6
                        textBox6.Text = total.ToString();
                    }
        ...
        private void txt_Add_Item_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var x = txt_Add_Item.SelectedItem.ToString();
                var allnames = mob_sys.Items.Where(s => s.Name == x).Select(s => new
                {
                    s.Notes,
                    s.ID,
                ...
                var pa_price = mob_sys.Sale_check_details.Where(s => s.Sale_check_id == s_id && s.Item_ID == allnames.ID).Select(s => s.Paid_price_item).SingleOrDefault();
                ...
                var sale_count = mob_sys.Sale_check_details.Where(s => s.Sale_check_id ==check_idd && s.Item_ID==allnames.ID ).Select(s => s.count).SingleOrDefault();
                textBox5.Text = sale_count.ToString();
                int quantity = 0;
                if (isSaleidexistinReturncheck(s_id))
                {
                    var r_id = mob_sys.Returns_checks.Where(s => s.sale_check_id == s_id ).Select(s => s.ID).ToList();
                    ...
                    foreach (var item in r_id)
                    {
                        if (isItemidexistinReturncheck(allnames.ID, item))
                        {
                            ...
                            var count_return = mob_sys.Returns_check_details.Where(s => s.Item_ID == allnames.ID && s.Returns_check_id == item).Select(s => s.count).SingleOrDefault();
        ...
        public bool isSaleidexistinReturncheck(int i)
        {
            try
            {
                ...
                var allitems = mob_sys.Returns_checks.Select(s => s.sale_check_id).ToList();
                foreach (var item in allitems)
                {
                    if (item == i)
                    {
        ...
        public bool isItemidexistinReturncheck(int i, int ch_id)
        {
            try
            {
                ...
                var allitems = mob_sys.Returns_check_details.Where(s => s.Returns_check_id == ch_id).Select(s => s.Item_ID).ToList();
                foreach (var item in allitems)
                {
                    if (item == i)
                    {
                ...
                if (textBox11.Text != "" && textBox14.Text != "")
                {
                    if (textBox6.Text != "")
                    {
                        if (float.Parse(textBox14.Text) <= float.Parse(textBox6.Text))
                        {
                            ...
                            try
                            {
                                var maxid = mob_sys.Returns_checks.Max(s => s.ID);
                                var max = maxid + 1;

                                mob_sys.insert_return_check(max, d, subname,
                            ...
                            {
                                mob_sys.insert_return_check(1, d, subname, Class1.TheID, egmaly, int.Parse(textBox2.Text), n, diff);
                                ins_item();
                                label8.Text = "تم عمل الفاتوره";
                                button1.Text = "تم الحفظ";
                                button4.Enabled = true;
        ...
        private void ins_item()
        {
            if (dataGridView1.RowCount != -1)
            {
                for (int i = 0; i < dataGridView1.RowCount; i++)
                {
                    ...
                    var max_return_id = mob_sys.Returns_checks.Max(s => s.ID);
                    try
                    {
                        var return_id = mob_sys.Returns_check_details.Max(s => s.ID);
                        mob_sys.ins_returns_check_detail(return_id + 1, item_id, count, pricee, "", max_return_id);
                        mob_sys.plus_item_count(count,item_id);
                    }
                    catch (Exception)
                    {
                        mob_sys.ins_returns_check_detail(1, item_id, count, pricee, " ", max_return_id);
                        mob_sys.plus_item_count(count, item_id);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (textBox14.Text != "" && textBox15.Text != "" && textBox6.Text != "")
            {
                if (float.Parse(textBox14.Text) <= float.Parse(textBox6.Text))
                {
                    if (button1.Text == "تم الحفظ")
                    {
                        Class1.Return_id= mob_sys.Returns_checks.Max(s => s.ID);
                        View_report_return_sale_check v = new View_report_return_sale_check();
                        v.Show();
                    }
                    else
                    {
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            if (ifsale_to_exist())
            {
                ...
                this.Close();
                allchecks_by_name all_checks = new allchecks_by_name();
                all_checks.Show();
        ...
        private bool ifsale_to_exist()
        {
            try
            {
                ...
                var all_names = mob_sys.Sale_checks.Select(s => s.Sale_To).ToList();
