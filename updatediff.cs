...
{
    public partial class updatediff : Form
    {
        ...
        public void load_details() {
            try
            {
                ...
                var check_details = mob_sys.Sale_checks.Where(s => s.ID == ch_id).Select(s => new { s.Total_price, s.difference_col, s.Sale_To,s.remaining_val }).SingleOrDefault();
        ...
        private void updatediff_Load(object sender, EventArgs e)
        {
            ...
            if (textBox1.Text != "")
            {
                if (textBox5.Text != "")
                {
                    ...
                    if (p <= total && p <= diff && p != 0)
                    {
                        try
                        {
                            var max = mob_sys.Payment_Ins.Max(s => s.ID);
                            mob_sys.ins_payment_in(max + 1, Class1.TheID, System.DateTime.Now.Date, p, textBox6.Text);

                            mob_sys.update_diff(i, p);
                            label9.Text = "تم السداد ";
                            load_details();
                        }
                        catch (Exception)
                        {
                            mob_sys.ins_payment_in(1, Class1.TheID, System.DateTime.Now.Date, p, textBox6.Text);
                            mob_sys.update_diff(i, p);
                            label9.Text = "تم السداد ";
                            load_details();
                        }
        ...
        private void textBox6_TextChanged(object sender, EventArgs e)
        {
            try
            {
                var all_id = mob_sys.Sale_checks.Where(s => s.Sale_To == textBox6.Text).Select(s => s.ID).ToList();
