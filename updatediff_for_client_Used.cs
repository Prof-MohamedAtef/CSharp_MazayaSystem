...
{
    public partial class updatediff_for_client_Used : Form
    {
        DataClasses1DataContext mob_sys = new DataClasses1DataContext();

        public updatediff_for_client_Used()
        {
            InitializeComponent();
        }
        public void load_details() {
            try
            {
                ...
                var check_details = mob_sys.Used_Return_checks.Where(s => s.ID == ch_id).Select(s => new { s.Total_Price, s.difference_col, s.Return_from }).SingleOrDefault();
            ...
            if (textBox1.Text != "")
            {
                if (textBox5.Text != "")
                {
                    ...
                    if (p <= total && p <= diff && p != 0)
                    {
                        try
                        {
                            var max = mob_sys.Payment_Return_Clients_Useds.Max(s => s.ID);
                            mob_sys.payment_Return_Client_Used(max + 1, Class1.TheID, System.DateTime.Now.Date, p, textBox6.Text);

                            mob_sys.update_diff_return_clients_Used(i, p);
                            label9.Text = "تم السداد ";
                            load_details();
                        }
                        catch (Exception)
                        {
                            mob_sys.payment_Return_Client_Used(1, Class1.TheID, System.DateTime.Now.Date, p, textBox6.Text);
                            mob_sys.update_diff_return_clients_Used(i, p);
                            label9.Text = "تم السداد ";
                            load_details();
                        }
        ...
        private void textBox6_TextChanged(object sender, EventArgs e)
        {
            try
            {
                var all_id = mob_sys.Used_Return_checks.Where(s => s.Return_from == textBox6.Text).Select(s => s.ID).ToList();
        ...
        private void updatediff_for_client_Used_Load_1(object sender, EventArgs e)
        {
