...
{
    public partial class Search_Check : Form
    {
        ...
        private void Search_Check_Load(object sender, EventArgs e)
        {
        ...
        private void Search_btn_Click(object sender, EventArgs e)
        {
            ...
            if (radio_date.Checked)
            {
                    ...
                    var date_picker = dateTimePicker1.Value.Date;
                        var prev = mob_sys.Sale_checks.Where(s => s.sale_Date == date_picker).Select(s => new
                        {
                            s.ID,
                            s.Notes,
                            s.Sale_To,
                        ...
                        var pursh_prev = mob_sys.Purchase_checks.Where(s => s.Recieve_Date == date_picker).Select(s => new
                        {
                            s.ID,
                            s.Notes,
                            s.Supplier_Name,
                        ...
                        if (pursh_prev.Count!=0)
                        {
                            ...
                            {
                                var casheirName = mob_sys.login_Tbls.Where(s => s.ID == item.Casher_id).Select(s => s.UserName).SingleOrDefault();
            ...
            else if (radio_id.Checked)
            {
                try
                {
                    ...
                    try
                    {
                        var prev = mob_sys.Sale_checks.Where(s => s.ID == ch_id).Select(s => new
                        {
                            s.ID,
                            s.Notes,
                            s.Sale_To,
                    ...
                    try
                    {
                        var pursh_prev = mob_sys.Purchase_checks.Where(s => s.ID == ch_id).Select(s => new
                        {
                            s.ID,
                            s.Notes,
                            s.Supplier_Name,
        ...
        public void load_sale_grid_details( int i) {
            c = 1;
            dt3.Clear();
            var sale_details = mob_sys.Sale_check_details.Where(s => s.Sale_check_id==i).Select(s => new
            {
               s.count,
               s.Item_ID,
               s.Notes,
            ...
            {
                var item_name = mob_sys.Items.Where(s => s.ID == item.Item_ID).Select(s => s.Name).SingleOrDefault();
                var comp_type_id = mob_sys.Items.Where(s => s.ID == item.Item_ID).Select(s => new { s.Producing_company_id,s.Type }).SingleOrDefault();
        ...
        public void load_purchase_grid_details(int i)
        {
            ...
            c = 1;
            dt3.Clear();
            var pursh_details = mob_sys.purchase_check_details.Where(s => s.purchase_check_id == i).Select(s => new
            {
                s.count,
                s.Item_ID,
                s.Notes,
