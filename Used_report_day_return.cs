...
{
    public partial class Used_report_day_return : Form
    {
        DataClasses1DataContext mob_sys = new DataClasses1DataContext();
        DataTable dt = new DataTable();
        DataTable dt2= new DataTable();
        public Used_report_day_return()
        {
            InitializeComponent();
        }

        private void Used_report_day_return_Load(object sender, EventArgs e)
        {
            ...
            int i = 1;
            var x = mob_sys.Used_return_checks_date(System.DateTime.Now.Date).ToList();
            foreach (var item in x)
            {
                dt.Rows.Add(i, item.ID, item.Return_from, item.UserName, item.Total_price, item.difference_col, item.diff, item.Notes);
                i++;
            }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var x = mob_sys.Used_return_details(id).ToList();
            int i = 1;
            foreach (var item in x)
            {
